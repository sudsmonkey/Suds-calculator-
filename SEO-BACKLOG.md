...
1. **`LocalBusiness` JSON-LD on `index.html`** — ✅ **done.** A
   `HomeAndConstructionBusiness` block sits in `<head>`, generated by the
   Reviews tab in `admin.html`, carrying `name`, `telephone`, `url`, `address`
   (125 NW 29th St, Oklahoma City, OK 73103), `areaServed` (twelve cities),
   `openingHoursSpecification` (Mon-Fri 8-5, Sat 9-2, closed Sunday), real
   `review` entries, and an `aggregateRating`.

   `priceRange` deliberately skipped — weak signal for this business type,
   doesn't feed the local-search relevance the other fields do, and risks
   going stale as pricing changes.

   The block is machine-generated (both in `<head>` and in the generator in
