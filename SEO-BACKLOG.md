...
1. **`LocalBusiness` JSON-LD on `index.html`** — ⚠️ **partly done.** A
   `HomeAndConstructionBusiness` block now sits in `<head>`, generated by the
   Reviews tab in `admin.html`, carrying `name`, `telephone`, `url`, four real
   `review` entries and an `aggregateRating`.

   Still missing the fields that answer *where*: **`areaServed`** (all ten
   cities — this is the home for Yukon, Mustang, Midwest City, Bethany,
   Piedmont and Choctaw, per the note above), **`priceRange`**, and
   **`openingHours`** once real hours are settled. An `address` would help too.

   The block is machine-generated, so add those fields to the generator in
   `admin.html` rather than hand-editing `<head>` — otherwise the next
   regenerate drops them.
